...
export default function Lotes() {
    ...
    const cargarDatos = async () => {
        ...
        try {
            ...
            alertaError('Error al cargar datos del sistema');
        } finally {
            setLoading(false);
        }
    };
    ...
    const lotesFiltrados = useMemo(() => {
        ...
        return lotes.filter(l => {
            ...
            const totalStock =
                l.detalles?.reduce((acc, d) => acc + (d.cantidad || 0), 0) || 0;
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.proveedorId || !formData.fechaVencimiento) {
            alertaError('Por favor complete los campos obligatorios');
            return;
        }
        ...
        try {
            ...
            alertaExito('Lote registrado correctamente');

            cargarDatos();

        } catch (error) {
            alertaError('Error al guardar lote');
        } finally {
            setLoading(false);
        }
    };
    ...
    const imprimirLote = async (lote) => {
        ...
        const proveedor =
            proveedores.find(p => p.id === lote.proveedorId)?.nombre ||
            'Desconocido';
    ...
    const loadUbicacionesForRack = async (rackId) => {
        ...
        try {
            ...
            console.error(error);
            alertaError('Error al cargar ubicaciones');
        } finally {
                        ...
                        {loading ? (
                            ...
                            paginatedLotes.map((l, idx) => {
                                ...
                                return (
                                    <tr
                                        key={l.id}
                                        className={`fade-in-row ${!l.activo ? 'row-inactive' : ''}`}
                                        style={{ animationDelay: `${idx * 0.05}s` }}
                                    >
                                        <td>
                                            {(() => {
                                                const med = medicamentos.find(
                                                    m => m.id === l.detalles?.[0]?.medicamentoId
                                                );
                                        ...
                                        <td>
                                            {proveedores.find(p => p.id === l.proveedorId)?.nombre || '—'}
                                        </td>
                                                ...
                                                {l.detalles?.map((det, i) => {
                                                    ...
                                                    return (
                                                        <div key={i} className="med-chip-with-img">
                                                            ...
                                                            <span>
                                                                    {med?.nombre || 'S/N'} <small>x{det.cantidad}</small>
                                                                </span>
                                                        </div>
                                                    );
                                                })}
                                ...
                                <div key={index} className="item-entry-row">
                                    ...
                                    <div className="location-selector-button">
                                        ...
                                        >
                                            {det.rackId
                                                ? `📍 ${
                                                    racks.find(r => r.id === det.rackId)?.nombre || '?'
                                                } N${det.nivel + 1} C${det.columna + 1}`
                                                : '📌 Ubicación'}
                                        </button>
                                    </div>
                                </div>
                ...
                <div className="drawer-overlay" onClick={closeLocationSelector}>
                    ...
                    >
                        <div className="modal-header">
                            ...
                            <button className="close-btn" onClick={closeLocationSelector}>
                                ×
                            </button>
                        </div>
                            ...
                            {!selectedRackForSelector ? (
                                ...
                                <div>
                                    <div className="selected-rack-info">
                                        ...
                                        <button onClick={() => setSelectedRackForSelector(null)}>
                                            Cambiar estante
                                        </button>
                                    </div>
